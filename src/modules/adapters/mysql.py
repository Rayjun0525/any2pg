...
class MySQLAdapter(BaseDBAdapter):
    def get_tables_and_views(self, schema: str = None) -> list[dict]:
        ...
        # MySQL은 schema 인자가 곧 database name
    ...
    def get_procedures(self, schema: str = None) -> list[dict]:
        # MySQL은 information_schema.ROUTINES 조회
        # schema 인자가 없으면 현재 연결된 DATABASE() 사용
        ...
        with self.engine.connect() as conn:
            ...
            for row in rows:
                ...
                results.append({
                    "name": r_name,
                    "type": r_type, # 'PROCEDURE' or 'FUNCTION'
                    "ddl": None,
                    "source": r_def # MySQL은 여기에 전체 코드가 들어있음
                })
