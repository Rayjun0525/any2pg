...
class RAGContextBuilder:
    ...
    def _extract_references(self, sql: str) -> Set[str]:
        refs = set()
        try:
            parsed_statements = sqlglot.parse(sql, read="oracle")
    ...
    def _fetch_metadata(self, names: Set[str]) -> List[Dict]:
        ...
        try:
            with self.db_mngr.get_cursor() as cur:
                ...
                for row in rows:
                    results.append({
                        "schema_name": row['schema_name'], # [추가]
                        "name": row['obj_name'],
                        "type": row['obj_type'],
                        "ddl": row['ddl_script'],
    ...
    def _format_output(self, metadata_list: List[Dict]) -> str:
        ...
        for meta in metadata_list:
            # [변경] 스키마명.객체명 형식으로 표기 (예: HR.EMPLOYEES)
            full_name = f"{meta['schema_name']}.{meta['name']}"
            
            if meta['type'] in ('TABLE', 'VIEW') and meta['ddl']:
                context_parts.append(f"-- Table/View: {full_name}")
                context_parts.append(meta['ddl'])
                
            elif meta['type'] in ('PROCEDURE', 'FUNCTION', 'PACKAGE') and meta['source']:
