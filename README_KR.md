...
3. **상태/조회**: *Status & browse*에서 **메타데이터 확인**(스키마 → 오브젝트 → DDL/소스), **포팅 현황**(상태 카운트 + 최신 렌더 결과), **실행 로그**(`project.silent_mode`/`--silent` 사용 시 로그 열람)에 접근합니다.
4. **포팅 실행/재개**: *포팅 실행/재개* 메뉴에서 **FAST(sqlglot+verifier)** 또는 **FULL(LLM+RAG)** 모드를 선택합니다. 기본값은 YAML 설정을 그대로 사용하며, 고급 필터(선택/변경 자산, 특정 파일명)는 필요할 때만 켜면 됩니다.
...
7. **품질 검사**: 메뉴에서 바로 품질 게이트를 실행해 점수/경고를 확인합니다.
8. **사일런트 모드**: `project.silent_mode: true` 또는 `--silent`로 표준출력을 최소화하고 실행 이벤트를 SQLite에 기록합니다.

9. **조작법**: ↑/↓로 이동하고, ←로 뒤로 가며, → 또는 Enter로 선택합니다. ESC도 메뉴를 닫습니다.
